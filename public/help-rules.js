...
(function() {
  ...
  const RULES = {
    ...
    '/v22/': { title: 'Đua Xe Tri Thức', rules: '• Đua với 3 xe bot (500m)\n• Đúng = tiến 50m, Sai = 10m\n• Combo 3+ = Boost 80m!\n• Bot trả lời ngẫu nhiên 60-70%\n• Về nhất = chiến thắng!' },
  ...
  function createHelpButton() {
    // Inject shared emoji-icon CSS (used by replaced emoji → img tags)
    if (!document.getElementById('hv-em-icon-style')) {
      const s = document.createElement('style');
      s.id = 'hv-em-icon-style';
      s.textContent = '.em-icon{width:1em;height:1em;vertical-align:-0.1em;display:inline-block;}';
    ...
    btn.innerHTML = '<img src="/img/help.png" style="width:36px;height:36px;">';
    btn.title = 'Luật chơi';
    btn.style.cssText = 'position:fixed;top:12px;right:12px;z-index:9000;width:38px;height:38px;border-radius:50%;border:none;background:transparent;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:transform 0.2s;';
  ...
  function injectHomeButton() {
    ...
    btn.innerHTML = '<img src="/img/exit.png" style="width:36px;height:36px;">';
    btn.title = 'Thoát game';
    btn.style.cssText = 'position:fixed;top:12px;left:12px;z-index:9000;width:38px;height:38px;border-radius:50%;border:none;background:transparent;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:transform 0.2s;';
