// V51 — Vườn Thú Pokémon Việt (controller)
(function () {
  'use strict';
  const STORAGE_KEY = 'v51_zoo';
  const QL = 28;
  let userData = { totalCaptures: 0, totalPokedex: 0 };
  ...
  let charRefs = {};            // { speciesId: { slot, char } } for caught creatures
  const $ = id => document.getElementById(id);
  const ss = id => { document.querySelectorAll('.screen').forEach(s => s.classList.remove('active')); $(id).classList.add('active'); };

  function renderStart() {
    $('total-captures').textContent = userData.totalCaptures;
    $('total-pokedex').textContent = userData.totalPokedex;
  }
  function wireSel() {
  ...
  function nextQ() {
    const p = window.V51Logic.pickNextQuestion({ cache, usedIds: used });
  ...
  async function startRun() {
    state = window.V51Logic.initState({ startedAt: Date.now() });
    cache = []; used.clear(); combo = 0; maxCombo = 0; fbId = -1; charRefs = {};
    ss('game-screen'); renderHud(); renderPokedex();
  ...
  function renderHud() {
    $('captured-text').textContent = `${state.captured.length}/${state.totalSpecies}`;
  ...
  function renderPokedex() {
    ...
    window.V51Logic.SPECIES.slice(0, state.totalSpecies).forEach(sp => {
      const caught = state.captured.includes(sp.id);
      const el = document.createElement('div');
      el.className = 'poke-slot' + (caught ? ' caught' : ' silhouette');
      const art = document.createElement('div');
      art.className = 'poke-art';
      el.appendChild(art);
      let char = null;
      if (C && C.hasSpecies(sp.id)) {
        // Mount the creature sprite. For uncaught slots the silhouette CSS
        // filter dims it into a shadow; caught slots reveal the full sprite.
        char = C.createCharacter(sp.id, art, { state: 'idle' });
      } else {
        // Emoji fallback if the sprite registry is missing.
        const e = document.createElement('span');
        e.textContent = sp.emoji;
        e.style.fontSize = '1.8rem';
        art.appendChild(e);
      }
      const n = document.createElement('span');
      n.className = 'name';
      n.textContent = caught ? sp.name : '???';
      el.appendChild(n);
      pd.appendChild(el);
      if (caught) charRefs[sp.id] = { slot: el, char };
    });
  }

  function showNextQ() {
    if (window.V51Logic.isFinished(state)) { finish(); return; }
    curQ = nextQ();
    locked = false;
    qStart = Date.now();
  ...
  function startTimer() {
    clearInterval(tH);
    const total = window.V51Logic.TIMER_SECONDS * 1000;
  ...
  function handleAns(sel) {
    ...
    const prevCaught = state.captured.length;
    if (ok) {
      state = window.V51Logic.applyCorrect(state);
      combo++; if (combo > maxCombo) maxCombo = combo;
    } else {
      state = window.V51Logic.applyWrongOrTimeout(state);
      combo = 0;
    }
    logAns(sel, ck, ok, Date.now() - qStart);
    const fb = $('feedback');
    fb.style.display = 'block';
    const freshCatch = state.captured.length > prevCaught;
    let newId = null;
    if (freshCatch) {
      newId = state.captured[state.captured.length - 1];
      const sp = window.V51Logic.SPECIES.find(s => s.id === newId);
    ...
    if (freshCatch && newId) {
      const ref = charRefs[newId];
      if (ref) {
        if (ref.char) ref.char.setState('happy');
        spawnParticles(ref.slot, 'sparkle', 10);
        setTimeout(() => { if (ref.char) ref.char.setState('idle'); }, 650);
      }
    }
    setTimeout(() => { if (window.V51Logic.isFinished(state)) finish(); else showNextQ(); }, 1100);
  }
  ...
  function finish() {
    ...
    userData.totalCaptures += state.captured.length;
    if (state.outcome === 'won') userData.totalPokedex += 1;
    saveData();
    let stars = 0;
    if (state.outcome === 'won') stars = 3;
    else if (state.captured.length >= 5) stars = 2;
    else if (state.captured.length >= 2) stars = 1;
    saveSession({ stars, acc, total });
    // Confetti burst on completing the pokedex.
    if (state.outcome === 'won') spawnConfetti($('zoo-stage') || document.body, 40);
  ...
  async function saveSession({ stars, acc, total }) {
    try {
      ...
      await fetch('/api/sessions', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({
        ...
        score: state.captured.length, total_questions: total, correct_answers: state.correct,
        stars_earned: stars, combo_max: maxCombo, mode: 'v51', accuracy: acc,
      })});
    } catch(e){}
  }
  ...
  function spawnConfetti(parent, count) {
    if (!parent) return;
    const colors = ['#ffeb3b', '#43a047', '#81c784', '#a5d6a7', '#ffca28', '#fff'];
    for (let i = 0; i < count; i++) {
      const p = document.createElement('span');
  ...
  function init() {
    if (!window.V51Logic) { setTimeout(init, 30); return; }
