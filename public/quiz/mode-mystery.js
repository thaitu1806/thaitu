...
(function () {
  ...
  reg('mystery', {
    ...
    render(ctx) {
      ...
      const wrongs = helpers.shuffle(helpers.wrongList(q)).slice(0, 3).map(o => o.text);
      ...
      choices.forEach((c, i) => {
        ...
        box.innerHTML = `<span class="qz-box-lid"><img src="/img/gift.png" style="width:100%;height:100%;object-fit:contain;"></span><span class="qz-box-val">${c.text}</span>`;
        box.addEventListener('click', () => {
          box.classList.add('open');
          box.classList.add(c.ok ? 'correct' : 'wrong');
          // Mở tất cả hộp còn lại
          optionsEl.querySelectorAll('.qz-box').forEach(b => {
            b.classList.add('open', 'disabled');
