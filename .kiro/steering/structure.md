...
- **Quiz answer engine (`public/quiz/`)**: pluggable answer engine. `public/quiz/engine.js` defines `window.HocVuiQuiz` with `registerMode(id, def)` + `render({ questionEl, optionsEl, question, onResult, mode? })`. Each question TYPE is a drop-in plugin file (`public/quiz/mode-*.js`) that calls `HocVuiQuiz.registerMode(...)` (or queues into `window.__hvQuizPending` if loaded before the engine). Current modes: `mode-choice` (multiple choice, applies to all), `mode-truefalse` (statement Đúng/Sai), `mode-type` (numeric keypad / text input; only when answer is short — `helpers.isShortAnswer`), `mode-tap` (tap the correct scattered chip). Modes apply `.option-btn`/`.correct`/`.wrong` so the shared sounds/mascot/collection/engagement observers fire. A game picks one applicable mode per question (weighted random); `render(...)` returns a handle with `revealTimeout()`. **Adding a new question type later = add a `mode-*.js` and include it; no game edits.** Piloted in **v48** (loads `/quiz/engine.js` + the four `mode-*.js` before `game.js`; `handleAns` takes a boolean `ok`). To apply to another game: include the same scripts before its `game.js`, replace its hand-built option buttons with `HocVuiQuiz.render(...)`, and make its answer handler accept `ok`.
