...
- `answer_logs.session_id` is **nullable** (FK to `game_sessions`). Games log each answer *during* play but the session row is only created when the game finishes, so `api/answers.js` + `server.js` insert `NULL` (not `0`) when no valid session id is present — otherwise the FK fails with a 500. Never pass `session_id: 0`. A self-healing migration in both `db/database.js` (`initDb`) and `api/db.js` rebuilds `answer_logs` to drop a legacy `NOT NULL` on `session_id` if an old DB still has it (SQLite can't `ALTER` away a column constraint). Both `/api/answers` handlers also coerce `selected_answer`/`correct_answer` to strings via `String(x ?? '')` because quiz-engine modes call the game's answer handler with a **boolean** `ok` (SQLite can't bind a JS boolean). NOTE: the better-sqlite3 wrapper in `db/database.js` routes `PRAGMA` statements through `.all()` (reader) so `PRAGMA table_info(...)` returns rows.
