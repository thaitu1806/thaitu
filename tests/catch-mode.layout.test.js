...
describe('catch mode layout', () => {
  ...
  test('each chip gets its own row (top set) and glides horizontally via the animation loop', () => {
    const js = read('mode-catch.js');
    // chip vertical placement uses per-index row
    expect(js).toMatch(/chip\.style\.top\s*=\s*\(\(\(i \+ 0\.5\)\s*\/\s*n\)/);
    // smooth sine-based horizontal drift centered at 50%
    expect(js).toMatch(/Math\.sin/);
    expect(js).toMatch(/50\s*\+\s*c\.amp\s*\*\s*Math\.sin/);
    // left is set as a percentage each frame
    expect(js).toMatch(/c\.el\.style\.left\s*=\s*x\s*\+\s*'%'/);
  });
});
